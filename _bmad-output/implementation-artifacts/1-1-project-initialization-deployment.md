...
- [ ] Task 1: Scaffold Next.js project (AC: #1)
  - [ ] Run `npx create-next-app@latest worldcup-app --typescript --tailwind --eslint --app --src-dir --turbopack --import-alias "@/*"`
  - [ ] Verify project runs with `npm run dev` and default page loads at `http://localhost:3000`
  - [ ] Verify TypeScript strict mode is enabled in `tsconfig.json`
  - [ ] Verify Turbopack is the default bundler (Next.js 16 default)

- [ ] Task 2: Initialize shadcn/ui and add components (AC: #2)
  - [ ] Run `npx shadcn@latest init` (accept defaults, confirm `src/` directory detection)
  - [ ] Run `npx shadcn@latest add button input table tabs switch`
  - [ ] Verify `components.json` exists at project root
  - [ ] Verify components exist in `src/components/ui/`

- [ ] Task 3: Install and configure database dependencies (AC: #3)
  - [ ] Run `npm install drizzle-orm @libsql/client`
  - [ ] Run `npm install -D drizzle-kit`
  - [ ] Create `drizzle.config.ts` at project root with Turso connection configuration
  - [ ] Create `src/db/index.ts` with Turso client + Drizzle instance setup
  - [ ] Create `src/db/schema.ts` as empty schema file (tables added in later stories)

- [ ] Task 4: Set up environment configuration (AC: #4)
  - [ ] Create `.env.example` with `TURSO_CONNECTION_URL`, `TURSO_AUTH_TOKEN`, `ADMIN_USERNAME`
  - [ ] Create `.env.local` with actual Turso credentials (user provides) and admin username
  - [ ] Verify `.env.local` is listed in `.gitignore` (Next.js includes this by default)

- [ ] Task 5: Set up project structure scaffold (AC: #1)
  - [ ] Create directory structure per architecture spec:
    ...
    - `src/lib/actions/`
    - `src/types/`
  - [ ] Create `src/types/index.ts` as empty types file
  - [ ] Create `src/lib/actions/types.ts` with `ActionResult<T>` type definition

- [ ] Task 6: Push to GitHub and deploy to Vercel (AC: #5)
  - [ ] Initialize git repo (if not already), commit all files
